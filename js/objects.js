/* Material-specific light, particles and motion. Canvas is decorative, never interactive. */
(function () {
  'use strict';
  const { clamp, orbitPosition } = window.MotionKit;
  const LETTERS = {
  ...
  class ObjectRenderer {
    constructor(button, config) {
      this.button = button; this.config = config;
      this.canvas = button.querySelector('canvas');
      this.ctx = this.canvas ? this.canvas.getContext('2d') : null;
      this.particles = []; this.reelAngle = 0; this.effectAge = 5;
      this.lastValue = null; this.lastEnergy = 0; this.size = { w: 0, h: 0 };
      this.resize();
    }
    resize() {
    ...
    frame(value, velocity, t, dt, reduced) {
      ...
      if (this.lastValue !== value || Math.abs(energy - this.lastEnergy) > .0001) {
        ...
        const card = this.button.closest('.object-card');
        ...
        this.lastValue = value; this.lastEnergy = energy;
      }
      this.effectAge += dt;
      ...
      switch (this.config.id) {
        case 'volt': this.volt(ctx, w, h, p, reduced ? 0 : t); break;
        case 'orbit': this.orbit(ctx, w, h, p, reduced ? 0 : t); break;
        case 'signal': this.signal(ctx, w, h, p, reduced ? 0 : t); break;
        case 'prism': this.prism(ctx, w, h, p, reduced ? 0 : t); break;
      }
      if (!reduced) this.drawParticles(ctx, dt);
    }
    volt(ctx, w, h, p, t) {
      if (p < .025) return;
      const end = 42 + 173 * p;
      ...
      glow.addColorStop(0, `rgba(80,167,245,${.17 * p})`); glow.addColorStop(1, 'rgba(60,145,255,0)');
      ...
      for (let strand = 0; strand < 3; strand++) {
        ...
        ctx.shadowBlur = strand === 0 ? 8 : 4; ctx.shadowColor = '#6baaff'; ctx.stroke();
      }
      [20, end].forEach(x => {
        ctx.fillStyle = `rgba(209,239,255,${p * .8})`; ctx.shadowBlur = 15;
        ctx.beginPath(); ctx.ellipse(x, h / 2, 2, 5 * p, 0, 0, Math.PI * 2); ctx.fill();
      });
      ctx.restore();
    }
    orbit(ctx, w, h, p, t) {
      ...
      for (let i = 0; i < 3; i++) {
        ...
        for (let j = 0; j < 18; j++) {
          ...
          ctx.beginPath(); ctx.arc(x, y, j === 0 ? 1.5 : .7, 0, Math.PI * 2); ctx.fill();
        }
      }
      ctx.restore();
    }
    prism(ctx, w, h, p, t) {
      ...
      for (let i = 0; i < 5; i++) {
        ...
        ctx.beginPath(); ctx.moveTo(x, 69); ctx.lineTo(endX - 13, 131); ctx.lineTo(endX + 13, 131); ctx.closePath(); ctx.fill();
      }
      ctx.restore();
    }
    signal(ctx, w, h, p, t) {
    ...
    drawParticles(ctx, dt) {
      for (const particle of this.particles) {
        particle.age += dt; particle.x += particle.vx * dt; particle.y += particle.vy * dt;
        particle.vx *= Math.exp(-dt * .8); particle.vy += dt * 7;
