...
test('32 redesigned A skins retain 48 total scroll/select skins and 563 current catalogue entries',()=>{
 assert.equal(bases.length,563);assert.equal(skins.length,48);assert.equal(expressive.length,32);
...
test('paper/paint/glass families are distinct structures, not a family of color-only overrides',()=>{
 const full=expressive.map(p=>read(p.base+'/styles.css'));assert.equal(new Set(full).size,32);
 for(const p of expressive){const css=read(p.base+'/styles.css');assert.match(css,/box-shadow/,p.id);assert.match(css,/::before|::after/,p.id);assert.match(css,/gradient/,p.id);}
