...
test('32 redesigned A skins retain 48 total scroll/select skins and 777 current catalogue entries',()=>{
 assert.equal(bases.length,777);assert.equal(skins.length,48);assert.equal(expressive.length,32);
