...
try {
  ...
  await check('entry keeps the first category lazy while listing 699 parts and 31 categories', async () => {
    assert.equal(await page.locator('#library-total').innerText(), '699');
    assert.equal(await page.locator('#library-collections').innerText(), '31');
