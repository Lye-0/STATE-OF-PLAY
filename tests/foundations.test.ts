...
test('reconstructed collection: 817 authored parts, 37 categories, 299 foundations',()=>{
 assert.equal(catalogue.parts.length,817);assert.equal(new Set(catalogue.parts.map(p=>p.category)).size,37);assert.equal(parts.length,299);
