...
test('reconstructed collection: 807 authored parts, 37 categories, 299 foundations',()=>{
 assert.equal(catalogue.parts.length,807);assert.equal(new Set(catalogue.parts.map(p=>p.category)).size,37);assert.equal(parts.length,299);
