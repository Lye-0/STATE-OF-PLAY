...
export function transformFixture(){
 ...
 const bases=(JSON.parse(read('src/catalog/registry.json')) as string[]).filter(p=>/^src\/parts\/(tabs|segments|popups|sliders)\//.test(p));
