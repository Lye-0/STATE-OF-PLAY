import {glassAlpha} from '../src/catalog/glass-transparency.ts';import fs from 'node:fs';import path from 'node:path';import assert from 'node:assert/strict';
...
try{
 ...
 for(const [id,category] of [['lg-lens-toggle','toggles'],['lgc-contextmenus-mist','contextmenus']] as const){
  ...
  const css=await content('/preview/styles.css');assert.ok(css.includes('--lg-alpha-scale:'+glassAlpha(70).scale));
  ...
  assert.equal(await demo.locator('.sop-'+id).evaluate(el=>Number(getComputedStyle(el).getPropertyValue('--lg-alpha-scale'))),glassAlpha(70).scale);
