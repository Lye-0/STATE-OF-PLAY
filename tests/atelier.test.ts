...
test('Atelier redesign covers all 171 new expressive parts except the existing loaders',()=>{
 assert.equal(parts.filter(p=>!p.tags.includes('KINETIC')&&!p.tags.includes('MOTION STUDIES')&&!p.tags.includes('GLASS LAB')).length,777);assert.equal(expressive.length,171);assert.equal(variants.length,16);
...
test('updated reproduction specifications explicitly include the new art-direction dependencies',()=>{
 for(const p of expressive){const prompt=read(p.base+'/prompt.md');if(p.tags.includes('WAYFINDER')){assert.match(prompt,/4\.10\.0/);assert.match(prompt,/wayfinding\//);assert.doesNotMatch(prompt,/atelier\.css/);continue;}if(p.tags.includes('SEQUENCE')){assert.match(prompt,/4\.9/);assert.match(prompt,/sequence\/style\.css/);assert.match(prompt,/sequence\/skin\.ts/);assert.doesNotMatch(prompt,/atelier\.css/);continue;}if(p.tags.includes('CONTINUUM')){assert.match(prompt,/4\.8/);assert.match(prompt,/continuum\/style\.css/);assert.doesNotMatch(prompt,/atelier\.css/);continue;}if(p.tags.includes('RESONANCE')){assert.match(prompt,p.category==='toasts'?/4\.13\.3/:/4\.7/);assert.match(prompt,/resonance\/style\.css/);if(p.category==='hints'){assert.match(prompt,/resonance\/hint-style\.css/);assert.match(prompt,/hint-art\.ts|art\.ts/);}else if(p.category==='toasts'){assert.match(prompt,/resonance\/notice-style\.css/);assert.match(prompt,/resonance\/notice-art\.ts/);}else assert.match(prompt,/resonance\/art\.ts/);assert.doesNotMatch(prompt,/atelier\.css/);continue;}assert.match(prompt,/4\.1/);assert.match(prompt,/atelier\.css/);assert.ok(prompt.includes(p.foundation.variant+'.css'));}
});
