...
test('Atelier redesign covers all 171 new expressive parts except the existing loaders',()=>{
 assert.equal(parts.filter(p=>!p.tags.includes('KINETIC')&&!p.tags.includes('MOTION STUDIES')).length,659);assert.equal(expressive.length,171);assert.equal(variants.length,16);
