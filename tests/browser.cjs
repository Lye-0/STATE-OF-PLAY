...
(async()=>{
 ...
 try{
  ...
  await run('File browser uses expandable folders and shows full source path',async()=>{
    const shared=page.locator('.source-directory[data-directory="src/shared"]');
    await shared.locator(':scope > summary').click();assert.equal(await shared.getAttribute('open'),null);
    await shared.locator(':scope > summary').click();assert.notEqual(await shared.getAttribute('open'),null);
    await page.locator('[data-file="src/shared/surface-controller.ts"]').click();
    assert.equal(await page.locator('.current-path').textContent(),'src/shared');
    await page.locator('[data-file$="/styles.css"]').click();
  });
  ...
  await run('Selected-file download returns exact source',async()=>{
   const item=await page.evaluate(()=>window.SOP_CATALOG.find(p=>p.id==='luminous-frame').files.tsx.find(f=>f.name.endsWith('/styles.css')));
  ...
  await run('Source and text ZIPs contain valid, unchanged component files',async()=>{
   ...
   for(const mode of ['source','text']){
    ...
    const source=read('packages/chrome/js/src/parts/toggles/chrome/vanilla/init.js');assert.equal(await z.file(root+'src/parts/toggles/chrome/vanilla/init.js'+(mode==='text'?'.txt':'')).async('string'),source);
    for(const dir of ['src/','src/parts/','src/parts/toggles/chrome/vanilla/','src/shared/','preview/'])assert.ok(z.files[root+dir]?.dir,dir);
    assert.ok((await page.locator('.package-tree').textContent()).includes('shared/'));
   }await page.keyboard.press('Escape');await page.keyboard.press('Escape');
  });
  await run('Mobile widths 320/390/768: no page overflow and usable detail actions',async()=>{
   for(const width of [320,390,768]){
    await page.setViewportSize({width,height:844});await page.locator('[data-open="chrome"]').click();await page.locator('[data-format="tsx"]').click();
    ...
    if(width<600){
      await page.locator('.mobile-file-picker select').selectOption('src/shared/motion.ts');
      assert.equal(await page.locator('.current-file').textContent(),'motion.ts');
    }
  ...
  if(!offline) await run('All 16 native JavaScript exports run with preserved relative imports',async()=>{
    const registry=JSON.parse(read('src/catalog/registry.json'));
    for(const base of registry){const meta=JSON.parse(read(base+'/meta.json'));await load(`packages/${meta.id}/js/${base}/vanilla/index.html`);
      const component=page.locator('.sop-'+meta.id).first();await component.waitFor();
      assert.ok((await component.boundingBox()).width>0);
      if(meta.category==='toggles'){const before=await component.getAttribute('aria-checked');await component.click();assert.notEqual(await component.getAttribute('aria-checked'),before);}
    }
  });
