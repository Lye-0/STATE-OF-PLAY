...
try{
 ...
 const oldLink=await browser.newPage();await oldLink.goto(new URL('?q=zz-missing-part',server.resolvedUrls!.local[0]).href,{waitUntil:'domcontentloaded',timeout:60000});await galleryReady(oldLink);assert.equal(await oldLink.locator('[data-part]').count(),24);assert.equal(await oldLink.locator('#search-parts').count(),0);await oldLink.close();assert.deepEqual(errors,[]);
