...
try{
 ...
 const advanced=await readValues(),phase=Array.from({length:phases.length},(_,n)=>n).find(n=>advanced.every((v,i)=>v===phases[(i+n)%phases.length]));assert.ok(phase&&phase>0,'all 26 indicators advance together through the loop');
 ...
 console.log('Progress demo checks: all 26 indicators, cyclic values, stop, manual interaction and category switch passed.');
