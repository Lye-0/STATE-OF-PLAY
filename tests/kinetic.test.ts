...
test('KINETIC adds 12 rails and 12 menus while retaining all non-KINETIC parts',()=>{
 ...
 for(const c of ['scrollbars','dropdowns']){assert.equal(added.filter(p=>p.category===c).length,12);assert.equal(all.filter(p=>p.category===c&&!p.tags.includes('GLASS LAB')).length,36);}
 assert.equal(new Set(all.map(p=>p.id)).size,all.length);assert.ok(added.every(p=>p.designType==='A'));
});
