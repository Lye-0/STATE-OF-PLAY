...
export function wayfinderFixture(){
 const read=(file:string)=>fs.readFileSync(path.join(ROOT,file),'utf8');
 const registry=JSON.parse(read('src/catalog/registry.json')) as string[];
 const records=registry.filter(base=>/^src\/parts\/(breadcrumbs|numbers)\//.test(base)).map(base=>({...JSON.parse(read(base+'/meta.json')),base,markup:read(base+'/markup.html')}));
...
window.mount(['aurora-trail']);`;
 ...
 const shell='<!doctype html><html lang="ja"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>WAYFINDER / component fixture</title><link rel="stylesheet" href="./styles.css"></head><body><form><main id="host"></main><button type="reset" id="reset">Reset</button></form><button type="button" id="outside">Outside</button></body></html>';
 const out=path.join(ROOT,'.test-output/wayfinder');fs.mkdirSync(out,{recursive:true});const entry='.test-output/wayfinder/fixture.ts';fs.writeFileSync(path.join(ROOT,entry),source);fs.writeFileSync(path.join(out,'styles.css'),styles+'\n'+frameCSS);fs.writeFileSync(path.join(out,'test.html'),shell.replace('</body>','<script type="module" src="./fixture.ts"></script></body>'));
