...
import {withGlassTransparency,glassDensity} from '../src/catalog/glass-transparency.ts';
...
test('all 70 glass templates export the chosen material without changing the template',()=>{
 ...
 for(const part of parts){
  const before=JSON.stringify(part),adjusted=withGlassTransparency(part,80);
