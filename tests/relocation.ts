...
for(const [index,parent]of topologies.entries())for(const layout of ['portable','original']as const){
 ...
 for(const [i,part]of parts.entries()){
  ...
  const art=part.category==='blocks'?part.markup.replace('<div class="sop-surface-content">','<div class="sop-surface-content"><button>Independent content</button>'):part.markup;
 ...
 write(prefix+'/index.html',`<!doctype html><html lang="ja"><head><meta charset="utf-8">${styles.join('')}<style>body{background:#191b20;color:#eee}section{padding:20px;min-height:180px}.sop-surface{width:330px}</style></head><body><button id="unmount">取り外す</button>${markup.join('')}<script type="module" src="./harness.js"></script></body></html>`);
 write(prefix+'/harness.js',imports.join('\n')+`\nconst controls=[${mounts.join(',')}];window.ready=true;document.querySelector('#unmount').onclick=()=>{controls.forEach(c=>c.destroy());document.querySelectorAll('section').forEach(s=>s.remove());window.removed=true;};`);
}
...
try{
 for(const fixture of fixtures){
  ...
  if (!offline) {
   ...
   for(const match of html.matchAll(/<link[^>]*href="([^"]+)"/g)) await page.addStyleTag({content:fs.readFileSync(path.join(out,fixture,match[1]),'utf8')});
  ...
  assert.equal(await page.locator('section').count(),parts.length);assert.deepEqual(failed,[]);
  for(const part of parts){const root=page.locator(`[data-part="${part.id}"] > *`);assert.ok((await root.boundingBox())!.width>0);if(part.category==='toggles'){const before=await root.getAttribute('aria-checked');await root.click();assert.notEqual(await root.getAttribute('aria-checked'),before);}}
