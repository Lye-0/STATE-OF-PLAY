...
try{
 ...
 await run('Mercury Selector: one-step selection keeps the metal reflection within a quiet range',async()=>{
  const r=page.locator('[data-part="mercury-segments"] .sop-mercury-segments'),inputs=r.locator('input[type="radio"]');await inputs.nth(0).check();await inputs.nth(0).focus();await page.keyboard.press('ArrowRight');assert.equal(await r.getAttribute('data-value'),'choice-2');
  const motion=await r.evaluate(async root=>{const plate=root.querySelector<HTMLElement>('.sop-transit-plate')!,angles:number[]=[];for(let i=0;i<24;i++){await new Promise<void>(resolve=>requestAnimationFrame(()=>resolve()));const image=getComputedStyle(plate).backgroundImage,angle=image.match(/linear-gradient\(\s*([-+\d.]+)deg/i);if(angle)angles.push(Number(angle[1]));}const style=getComputedStyle(root),face=getComputedStyle(plate);return{sweep:style.getPropertyValue('--tr-metal-sweep').trim(),sheenAngle:style.getPropertyValue('--tr-sheen-angle').trim(),sheenStrength:style.getPropertyValue('--tr-sheen-strength').trim(),stretch:style.getPropertyValue('--tr-metal-stretch').trim(),compression:style.getPropertyValue('--tr-metal-compression').trim(),angles,transform:face.transform};});
