...
候補の確定は即時です。開閉演出の完了を待つ必要はありません。ホバー中の背景と選択済みのチェックは別の状態です。閉じるときはARIA上の開状態を先に閉じ、約210msだけ残る面をinertかつaria-hiddenにします。途中で再度開いた場合は古い閉じ演出を中断します。ダミーの複製リストや重複IDは追加しません。
