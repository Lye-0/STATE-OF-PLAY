...
v4.10.10以降は、4モードともテキスト欄と独自の選択パネルを使います。日付欄や選択ボタンから同じカレンダーを開き、`time` と `datetime` では時・分の選択面を表示します。ブラウザー標準の日時ピッカーは開きません。時刻を直接入力する場合は `HH:mm`（例：`14:30`）を使えます。v4.12.2では期間の2つの日付を同じ幅で区切り線の左右に配置し、狭い部品幅では縦並びへ切り替えます。
