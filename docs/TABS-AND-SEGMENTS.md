...
表示項目は配列から描画します。選択マーカーは実際の要素サイズを計測し、`33.333%`や3番目までの固定CSSを使いません。多いタブは横方向へスクロールできます。 横向きタブ列の選択面が動いている間も縦スクロールバーは表示しません。縦向きのタブ配置は従来どおりです。横並びのセグメントは幅に応じて折り返し、選択マーカーも次の行へ移動します。縦並びも選択できます。
