...
各ガラス部品の詳細にある「素材を調整 → 透明度」で、展示テンプレートを起点に面の濃さを調整できます。50が展示の設定で、値を上げると背景が通りやすくなります。これは部品全体のopacityではなく、ガラスの背景色だけの調整です。文字、輪郭、影、無効状態の表示を保ち、持ち手・本体・展開パネルそれぞれの濃度差を維持します。

「背景のぼかし」は独立したスライダーです。0はぼかしなし、50は展示の設定、100は各面の基準の2倍です。`--lg-blur-scale` で背景のblurだけを乗算し、面と持ち手の元々のぼかしの差を保ちます。文字や光沢自体に使うfilterは変更しません。くっきりした背景を見せる場合は、透明度を上げ、背景のぼかしを下げてください。

Aタイプは従来の面の濃度の82%を新しい基準にしています。詳細での変更は展示カードへ反映されません。「展示の設定に戻す」で両方を50へ戻せます。

変更後のコード、コード込み／文章のみのプロンプト、使用例と依存を含むZIP、ZIP内の独立デモは同じ設定を使います。配布CSSの部品ルートに `--lg-alpha-scale`、`--lg-alpha-lift`、`--lg-blur-scale` を指定し、調整値は `INTEGRATION.json` の `glassTransparency` と `glassBlur` に記録します。両方が50なら通常のテンプレートをそのまま配布します。元のカタログデータを書き換えず、調整した配布データを生成します。

`--lg-density`（A: 0.82、B: 1）と `--lg-control-density` は展示時の濃淡を定める基準として保持します。詳細の透明度は、各面の基準アルファに共通の `alpha × --lg-alpha-scale + --lg-alpha-lift` を適用します。50はscale=1/lift=0、100は0.12/0、0は0.75/0.25。中間は線形補間し、濃淡の順序と差の比率を維持します。両端で全ての面が完全透明・不透明に潰れないようにし、持ち手や選択片も同じ計算で連動します。文字・輪郭・影・無効状態のopacityは変更しません。透過を減らす設定・強制配色・solid素材はこの調整より優先されます。背景の模様は展示と詳細の確認用で、配布パーツ本体には含みません。
