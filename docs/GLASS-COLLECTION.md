...
変更後のコード、コード込み／文章のみのプロンプト、使用例と依存を含むZIP、ZIP内の独立デモは同じ設定を使います。配布CSSの部品ルートに `--lg-density` と `--lg-blur-scale` を指定し、調整値は `INTEGRATION.json` の `glassTransparency` と `glassBlur` に記録します。両方が50なら通常のテンプレートをそのまま配布します。元のカタログデータを書き換えず、調整した配布データを生成します。

`--lg-density` は背景色のアルファ値に掛ける倍率です。Aの標準は0.82、Bは1で、詳細の0〜100は標準倍率の2〜0倍へ対応します。持ち手・選択片・進捗線には `--lg-control-density`（0.55 + 素材倍率 × 0.45）を使い、透明度を上げても操作位置や値が消えないよう、変化を緩やかにしています。透過を減らす設定・強制配色・solid素材はこの倍率より優先されます。背景の模様は展示と詳細の確認用で、配布パーツ本体には含みません。
