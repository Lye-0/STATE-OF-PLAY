...
Aタイプは従来の面の濃度の82%を新しい基準にしています。詳細での変更は展示カードへ反映されません。「展示の設定に戻す」で50へ戻せます。

変更後のコード、コード込み／文章のみのプロンプト、使用例と依存を含むZIP、ZIP内の独立デモは同じ設定を使います。配布CSSの部品ルートに `--lg-density` を指定し、調整値は `INTEGRATION.json` の `glassTransparency` にも記録します。値が50なら通常のテンプレートをそのまま配布します。元のカタログデータを書き換えず、調整した配布データを生成します。
