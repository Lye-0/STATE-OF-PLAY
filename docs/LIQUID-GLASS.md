...
v4.15.0で追加した4カテゴリ8作品。各カテゴリのAタイプとBタイプの並びに加わり、通常のカテゴリ選択とデザインフィルターで表示できます。

## 実装形式

従来どおりReact TSX/JSX、通常サイト向けTS/JS、導入向け/元の構成を選べます。ZIPには選択したパーツの素材CSS、動作、必要な共通ファイルを含めます。展示の背景やGLASS LAB操作UIは含めません。
...
詳細欄の背景／素材／文字配色／停止／実験屈折は、その場の比較設定です。設定切替で入力値やタブ本文を作り直しません。取得するコードは元ソースの既定値です。選んだ値を利用先にも使う場合は、上のprops/optionsを指定してください。
...
現在のリポジトリへ統合する際は、削除済みのStitch CometとPaper Loaderを戻さず、Hero Asteriskとカテゴリ別読み込みを維持しました。ロックファイルは依存パッケージを変えず、ルートのバージョンだけをv4.15.0へ合わせています。
