...
export function buildCatalog(root = ROOT): CatalogBuild {
  ...
  if (!Array.isArray(bases) || !bases.every((base): base is string => typeof base === 'string' && /^src\/parts\/(toggles|blocks|scrollbars|dropdowns|accordions|textboxes)\/[a-z0-9-]+$/.test(base)))
  ...
  const parts = bases.map(base => {
    ...
    if (!['toggles','blocks','scrollbars','dropdowns','accordions','textboxes'].includes(meta.category) || !Number.isFinite(meta.order) || !Array.isArray(meta.props) || !Array.isArray(meta.tags))
    ...
    const hint = meta.category === 'textboxes' ? '実際に入力できます。日本語・貼り付け・キーボード操作もそのまま使えます。入力値は保存・送信しません。' : meta.category === 'dropdowns' ? '開いて選ぶ。選択肢・説明・アイコンも差し替えられます。' : meta.category === 'accordions' ? '見出しで開閉。内側のコンテンツも自由に差し替えられます。' : meta.category === 'toggles' ? 'クリック・ドラッグ・キーボードで操作できます。' : meta.category === 'scrollbars' ? 'ホイール・スワイプ・レールのドラッグで読み進められます。' : '中身を自由に入れ替えられる、独立した背景パーツです。';
    const preview = {
