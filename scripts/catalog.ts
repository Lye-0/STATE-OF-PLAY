...
export function buildCatalog(root = ROOT, onlyIds?: readonly string[]): CatalogBuild {
  ...
  if (!Array.isArray(bases) || !bases.every((base): base is string => typeof base === 'string' && /^src\/parts\/(toggles|blocks|ornaments|scrollbars|dropdowns|accordions|textboxes|buttons|links|tabs|segments|checkboxes|popups|sliders|radios|comboboxes|toasts|hints|progress|loaders|uploads|datepickers|pagination|breadcrumbs|badges|numbers|avatars|ratings|colors|skeletons|timelines|wizards|searchbars|commands|contextmenus|navigation|tables)\/[a-z0-9-]+$/.test(base)))
