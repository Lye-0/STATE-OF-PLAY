...
export function buildCatalog(root = ROOT, onlyIds?: readonly string[]): CatalogBuild {
  ...
  const parts = bases.filter(base => !onlyIds || onlyIds.includes(base.split('/').at(-1)!)).map(base => {
    ...
    if (!['toggles','blocks','scrollbars','dropdowns','accordions','textboxes','buttons','links','tabs','segments','checkboxes','popups','sliders','radios','comboboxes','toasts','hints','progress','loaders','uploads','datepickers','pagination','breadcrumbs','badges','numbers','avatars','ratings','colors','skeletons','timelines','wizards'].includes(meta.category) || !Number.isFinite(meta.order) || !Array.isArray(meta.props) || !Array.isArray(meta.tags))
