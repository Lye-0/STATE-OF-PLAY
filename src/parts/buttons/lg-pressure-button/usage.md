# Pressure Button — Liquid Glass

光源が指先を追い、押すとガラス面だけが押し広がる。文字は静止したまま。
...
- `children / icon` (ReactNode): 表示するラベルと任意の小さな装飾
- `onClick / type` (native button props): 実際の処理へ接続。typeの初期値はbutton。
- `loading / disabled` (boolean): 処理中の再操作を防ぐ / 無効化
