# Prism Window — 再現仕様
結晶の面と光の線が重なる、スペクトルの窓。

styles.css/popup-base.css/popup-content.cssの色・寸法・余白・輪郭・文字組みと、examples内の内容構成を正本にする。ベース#292638、本文#f0e7fa、アクセント#d1c0f0。意図は「色の、入口。」。枠だけではなく内部のspectral / portalの情報表現も維持する。
