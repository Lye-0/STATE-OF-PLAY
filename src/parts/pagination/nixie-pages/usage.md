# Nixie Pages / SEQUENCE

ガラスの内側の電極が走り、現在のページを琥珀色に照らします。

## 組み込み

React版は`NixiePages`をimportします。`value`を渡すと外部制御、`defaultValue`だけなら内部制御です。通常HTML版は`markup.html`のルート要素を配置し、`styles.css`を読み込み、`init(element, options)`を実行します。不要になったら`destroy()`を呼びます。
