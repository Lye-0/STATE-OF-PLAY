# Copper Pages / SEQUENCE

薄い銅板がしなり、選択された数字の下で光が折れます。

## 組み込み

React版は`CopperPages`をimportします。`value`を渡すと外部制御、`defaultValue`だけなら内部制御です。通常HTML版は`markup.html`のルート要素を配置し、`styles.css`を読み込み、`init(element, options)`を実行します。不要になったら`destroy()`を呼びます。
