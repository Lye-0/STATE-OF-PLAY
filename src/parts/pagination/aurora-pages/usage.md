# Aurora Pages / SEQUENCE

透明な選択膜が伸びながら移り、面の光が追いかけます。

## 組み込み

React版は`AuroraPages`をimportします。`value`を渡すと外部制御、`defaultValue`だけなら内部制御です。通常HTML版は`markup.html`のルート要素を配置し、`styles.css`を読み込み、`init(element, options)`を実行します。不要になったら`destroy()`を呼びます。
