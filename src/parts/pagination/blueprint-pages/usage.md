# Blueprint Pages / SEQUENCE

測定枠が伸びて移動し、寸法線が次のページへつながります。

## 組み込み

React版は`BlueprintPages`をimportします。`value`を渡すと外部制御、`defaultValue`だけなら内部制御です。通常HTML版は`markup.html`のルート要素を配置し、`styles.css`を読み込み、`init(element, options)`を実行します。不要になったら`destroy()`を呼びます。
