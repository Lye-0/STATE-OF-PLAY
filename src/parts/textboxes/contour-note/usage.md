...
## 入力を組み込む
本物の `textarea` を使う入力パーツです。ラベル・placeholder・説明は、利用先の項目に合わせて差し替えます。Reactの本体に展示文言は固定していません。
...
## v2.0: 入力面の反応
公開APIは変更していません。ネイティブの文字入力を保ち、フォーカスと入力確定に合わせて装飾面だけを変化させます。`responsive-field`関連ファイルは必要な依存としてZIPへ含まれます。入力内容の保存・送信・実行はしません。
