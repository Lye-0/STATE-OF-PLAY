# Terminal Field — 入力可能なテキストボックスの再現仕様

デザイン分類: A。PHOSPHOR / CONSOLE。燐光のプロンプト、刻まれた目盛り。入力は実行されずテキストとして扱う。
思考を、起動。 ネイティブのtext inputを主役に、読みやすい16px以上の文字と静かなフォーカス反応を維持する。
