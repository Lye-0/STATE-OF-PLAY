# Copper Tags / SEQUENCE

銅箔の端がしなり、細い反射線が折り目を渡ります。

## 組み込み

React版は`CopperTags`をimportします。`value`を渡すと外部制御、`defaultValue`だけなら内部制御です。通常HTML版は`markup.html`のルート要素を配置し、`styles.css`を読み込み、`init(element, options)`を実行します。不要になったら`destroy()`を呼びます。
