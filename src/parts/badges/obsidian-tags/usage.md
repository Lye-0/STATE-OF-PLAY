# Obsidian Tags / SEQUENCE

石の面が左右に分かれ、裂け目に淡い光が現れます。

## 組み込み

React版は`ObsidianTags`をimportします。`value`を渡すと外部制御、`defaultValue`だけなら内部制御です。通常HTML版は`markup.html`のルート要素を配置し、`styles.css`を読み込み、`init(element, options)`を実行します。不要になったら`destroy()`を呼びます。
