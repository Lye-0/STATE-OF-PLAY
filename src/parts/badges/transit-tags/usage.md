# Transit Tags / SEQUENCE

切符のミシン目が緩み、削除時には端から紙が切り離れます。

## 組み込み

React版は`TransitTags`をimportします。`value`を渡すと外部制御、`defaultValue`だけなら内部制御です。通常HTML版は`markup.html`のルート要素を配置し、`styles.css`を読み込み、`init(element, options)`を実行します。不要になったら`destroy()`を呼びます。
