# Bloom Select — Liquid Glass

フィールドの近くからガラスの面が広がり、光を含んだ選択面が候補へ移ります。
...
material='clear'。Reactのappearanceはauto、同梱HTML例はdarkを明示しています。optics='standard'、paused=false。--lg-accentをRGB空白区切りで上書きできます。
...
- `items` (readonly SelectItem[]): value/label/description。アイコンは必要な場合だけ明示。
- `value / defaultValue / onValueChange` (string / callback): 選択値の外部制御 / 内部制御
- `label / name / disabled` (string / string / boolean): アクセシブル名、フォーム名、無効状態
