...
光のレンズを結ぶ手順表示と、余白を備えた透明な入力面で進めるウィザード。
...
Reactのpropsは `react/LgcWizardsLens.tsx` がexportする `LgcWizardsLensProps`、通常DOMは `init(root, options)` の型を参照してください。
