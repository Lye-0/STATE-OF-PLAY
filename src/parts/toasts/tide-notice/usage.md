...
Reactは同梱のTideNoticeを読み込み、controllerRefからnotify()・dismiss()・setPaused()を呼びます。通常DOM版はmarkup.htmlとstyles.cssを配置し、init(element, options)から得たコントローラーで同じ操作を行い、不要になったらdestroy()します。
通知の発行元となる通信・保存・検証は利用先で接続してください。Reactが管理する外側とコントローラーが管理する内側のDOMを分けます。

## 通知専用の表現 — v4.13.3

下縁の水位が上がって知らせが浮かび、静かに止まる。通知の見出し・説明・アクションはnotify()へ渡した値だけを表示します。
