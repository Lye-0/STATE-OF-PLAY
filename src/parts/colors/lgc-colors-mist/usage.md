...
濃い半透明の面に色の調整と確認をまとめたカラーピッカー。明色表示では面と文字色を一緒に切り替えます。

## 内容と状態の接続
選択値は不透明sRGB。彩度と明度の平面はポインター用、同じ値をネイティブrangeでキーボード操作可能。色相の輪は位置に応じて変化。無効なHEX入力は勝手に書き換えずエラー。

Reactのpropsは `react/LgcColorsMist.tsx` がexportする `LgcColorsMistProps`、通常DOMは `init(root, options)` の型を参照してください。
