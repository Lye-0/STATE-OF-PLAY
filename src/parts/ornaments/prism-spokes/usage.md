...
`PrismSpokes.tsx` を import して配置します。

```tsx
import PrismSpokes from './PrismSpokes';

export default function Example() {
  return <PrismSpokes paused={false} />;
}
```
...
const element = document.querySelector<HTMLElement>('[data-ornament="prism-spokes"]');
if (element) {
