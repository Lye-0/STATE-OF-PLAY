...
星をそれぞれ丸い透明レンズに浮かべ、選択位置は光の縁と番号で示します。

## 内容と状態の接続
実際のradioとlabelを用います。Space・矢印キーとフォーム値を維持。ホバーはプレビューであり値の変更ではありません。未評価は0。

Reactのpropsは `react/LgcRatingsLens.tsx` がexportする `LgcRatingsLensProps`、通常DOMは `init(root, options)` の型を参照してください。
