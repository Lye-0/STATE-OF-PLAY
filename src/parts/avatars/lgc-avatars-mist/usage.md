...
一枚の霧ガラスに人物を行として並べます。小さなイニシャル窓、氏名・役割、行番号と在席状態を保ち、選択行の縁を静かに照らします。

## 内容と状態の接続
画像が読み込めないときはイニシャルを表示。氏名や在席状態を外部から渡し、存在しないオンライン状態を捏造しない。ユーザーの選択とプロフィール表示は別の機能です。

Reactのpropsは `react/LgcAvatarsMist.tsx` がexportする `LgcAvatarsMistProps`、通常DOMは `init(root, options)` の型を参照してください。
