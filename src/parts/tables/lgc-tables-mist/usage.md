...
見出しからページ送りまでを一枚の濃いフロストガラス面にまとめます。セルは白く塗りつぶさず、文字の可読性と表内の横スクロールを維持します。
...
## 組み込み
本体フォルダーを既存のコンポーネント置き場へ配置してください。Reactは`LgcTablesMist`、通常サイトは`init(root, options)`を使用します。CSSとinternalの必要ファイルを一緒に配置してください。デモ文言やデータは変更できます。examplesとpreviewは実行時に不要です。
