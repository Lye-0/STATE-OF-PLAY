...
import type { Part, Format, DetailTab, PartController } from '../catalog/types';
import { isFormat, isDetailTab } from '../catalog/types';
export function createDetails(parts: Part[], callbacks: {onActive: (active: boolean) => void; onNavigate: (id: string, trigger?: HTMLElement) => void}) {
    ...
    function drawMain() {
        ...
        if (tab === 'code')
            mountCodeViewer(pane, part.files[format], { selected: sourceNames[part.id + format], onSelect: (name: string) => sourceNames[part.id + format] = name });
        if (tab === 'guide') {
        ...
        if (tab === 'prompt') {
            pane.innerHTML = `<div class="prompt-scroll"><div class="section-kicker">TAKE THE INTENTION WITH YOU</div><h3>見た目だけでなく、意図も渡す。</h3><p class="guide-lead">新しいAIとの会話に、そのまま貼り付けられる再現・組み込み指示です。</p><div class="prompt-modes"><button type="button" data-prompt-mode="full" aria-pressed="${copiedWithCode}">${icon('code')}<span><b>コード込み</b><small>再現性を優先する標準形式</small></span><i>推奨</i></button><button type="button" data-prompt-mode="spec" aria-pressed="${!copiedWithCode}">${icon('spark')}<span><b>再現仕様のみ</b><small>別の構成で作り直すときに</small></span></button></div><div class="prompt-toolbar"><span id="prompt-size"></span><button type="button" class="small-button" id="copy-prompt">${icon('copy')}プロンプトをコピー</button></div><textarea id="prompt-text" readonly spellcheck="false" aria-label="AI用の再現・組み込みプロンプト"></textarea><p class="prompt-caution">コード込みには必要ファイルと具体的な仕様が入ります。AIの出力そのものを保証するものではありません。APIへの送信は行いません。</p></div>`;
        ...
        required('#package-label', dialog).textContent = `${part.files[format].length} FILES · ${format.toUpperCase()}`;
    }
    ...
    function open(id: string, trigger?: HTMLElement) {
        ...
        dialog.innerHTML = `<div class="detail-shell"><header class="detail-top"><div class="detail-breadcrumb"><span class="mini-brand" aria-hidden="true">◒</span>THE COLLECTION<span>/</span>${toggle ? 'TOGGLES' : 'SURFACES'}<span>/</span><strong>${escapeHTML(part.id)}</strong></div><div class="detail-nav"><button type="button" class="icon-button" id="previous-part" aria-label="前のパーツ">${icon('left')}</button><button type="button" class="icon-button" id="next-part" aria-label="次のパーツ">${icon('right')}</button><span class="nav-divider"></span><button type="button" class="icon-button close-detail" aria-label="詳細を閉じる">${icon('close')}</button></div></header><div class="detail-body"><aside class="preview-sidebar"><div class="preview-heading"><span class="section-kicker">${String(part.order).padStart(2, '0')} / ${escapeHTML(part.material)}</span><h2 id="detail-title">${escapeHTML(part.name)}</h2><p>${escapeHTML(part.tagline)}</p></div><div class="live-preview ${toggle ? 'toggle-preview' : 'block-preview'} bg-${background}" data-preview-part="${escapeHTML(part.id)}"><div class="preview-status"><span><i></i>LIVE PREVIEW</span><span id="detail-state">${toggle ? 'ON' : 'HOVER / TOUCH'}</span></div><div class="preview-stage"></div><div class="preview-bottom"><span>${toggle ? 'CLICK OR DRAG' : 'MOVE YOUR POINTER'}</span><div class="background-picker" aria-label="プレビューの背景"><button type="button" data-bg="studio" aria-label="展示の背景" title="展示の背景"></button><button type="button" data-bg="dark" aria-label="黒い背景" title="黒い背景"></button><button type="button" data-bg="light" aria-label="明るい背景" title="明るい背景"></button></div></div></div>${toggle ? `<div class="preview-controls"><div class="state-segments" aria-label="プレビューの状態"><button type="button" data-state="off">OFF</button><button type="button" data-state="on">ON</button></div><button type="button" class="small-button" id="preview-loop" aria-pressed="false">${icon('play')}ループ</button><button type="button" class="icon-button" id="reset-preview" aria-label="初期状態に戻す">${icon('reset')}</button></div><label class="disabled-control"><input type="checkbox" id="preview-disabled"><span>無効状態を確認する</span></label>` : `<p class="surface-note">中身は自由に差し替えられます。<br>展示用の文字や番号はパーツ本体に含みません。</p>`}<p class="preview-description">${escapeHTML(part.description)}</p><dl class="part-facts"><div><dt>CATEGORY</dt><dd>${toggle ? 'Toggle / スイッチ' : 'Surface / コンテナ'}</dd></div><div><dt>MOTION</dt><dd>${escapeHTML(part.motion)}</dd></div><div><dt>VERSION</dt><dd>${escapeHTML(part.version)}</dd></div><div><dt>RUNTIME</dt><dd>${toggle ? 'CSS + SVG / Canvas' : 'CSS + Pointer'}</dd></div></dl><div class="related-box"><span class="section-kicker">${toggle ? 'PAIR IT WITH' : 'RELATED PART'}</span>${part.related.map(id => { const p = parts.find(p => p.id === id); return p ? `<button type="button" data-related="${id}"><span>${escapeHTML(p.name)}<small>${id === 'original-surface' ? 'この展示に使っている背景' : '組み合わせて使うパーツ'}</small></span>${icon('arrow')}</button>` : ''; }).join('')}</div></aside><section class="detail-main"><div class="detail-tabs" role="tablist" aria-label="詳細情報"><button id="tab-code" type="button" role="tab" aria-controls="detail-pane" data-detail-tab="code">${icon('code')}コード<span>CODE</span></button><button id="tab-guide" type="button" role="tab" aria-controls="detail-pane" data-detail-tab="guide">${icon('book')}使い方</button><button id="tab-prompt" type="button" role="tab" aria-controls="detail-pane" data-detail-tab="prompt">${icon('spark')}AI用プロンプト</button></div><div class="format-area"><div class="format-buttons" aria-label="実装形式">${Object.entries(FORMATS).map(([key, value]) => `<button type="button" data-format="${key}" aria-pressed="false">${value.short}</button>`).join('')}</div><p id="format-note"></p></div><div id="detail-pane" role="tabpanel"></div><footer class="detail-actions"><span id="package-label"></span><span class="package-hint">依存ファイル・使用例をまとめて取得</span><button type="button" class="solid-button" id="download-part">${icon('down')}パーツZIP</button></footer></section></div></div>`;
        required('.close-detail', dialog).addEventListener('click', close);
        required('#previous-part', dialog).addEventListener('click', () => callbacks.onNavigate(parts[(parts.indexOf(part) - 1 + parts.length) % parts.length].id));
        required('#next-part', dialog).addEventListener('click', () => callbacks.onNavigate(parts[(parts.indexOf(part) + 1) % parts.length].id));
        ...
        controller = mounts[part.id](root, part.category === 'toggles' ? { checked: part.initial, onCheckedChange: updatePreviewState } : {});
