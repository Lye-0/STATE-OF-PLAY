...
export function createDetails(parts: Part[], callbacks: {onActive: (active: boolean) => void; onNavigate: (id: string, trigger?: HTMLElement) => void}) {
    ...
    function drawMain() {
        ...
        if (tab === 'guide') {
            ...
            const options = react ? part.props : (part.category === 'buttons' || part.category === 'links') ? actionGuide(part.category) : part.category === 'textboxes' ? textFieldGuide() : (part.category === 'dropdowns' || part.category === 'accordions' || part.category === 'scrollbars') ? disclosureGuide(part.category) : part.category === 'toggles' ? [
