import type { ToggleConfig } from '../shared/toggle-controller';
export class TactileAudio {
    ...
    tone(freq: number, end: number, duration: number, gain: number, type: OscillatorType = 'sine', delay = 0) {
    ...
    noise(duration: number, gain: number, frequency = 3000, delay = 0) {
        const ctx = this.context;
        if (!ctx)
            return;
        if (!this.noiseBuffer) {
    ...
    play(config: Pick<ToggleConfig, 'id' | 'tone'>, on: boolean) {
