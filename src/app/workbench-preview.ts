...
export function mountWorkbenchSample(root:HTMLElement,part:PartPreview,api:PartController){
 ...
 // Navigation already marks the active destination; keep demo clicks local without a second caption.
 if(part.category==='navigation'){
  api.updateWorkbench?.({onNavigate:()=>false});
  return ()=>{};
 }
