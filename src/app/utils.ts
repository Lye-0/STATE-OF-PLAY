export const escapeHTML = (value: unknown): string => String(value ?? '').replace(/[&<>"']/g, char => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#39;' }[char as '&' | '<' | '>' | '"' | "'"] ?? char ?? char));
...
export function icon(name: string, cls = '') { return `<svg class="icon ${cls}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">${paths[name] ?? paths.code}</svg>`; }
let toastTimer = 0;
...
interface Feedback { original: string; label: string | null; timer: number; token: symbol; }
const feedback = new WeakMap<HTMLElement, Feedback>();
export function resetCopyFeedback(button: HTMLElement) {
...
function manualCopy(text: string, origin: HTMLElement | null) {
    document.querySelector<HTMLDialogElement>('dialog.manual-copy')?.close();
    ...
    const field = required('textarea', modal);
    field.value = text;
    required('button', modal).addEventListener('click', () => modal.close());
...
export async function copyText(text: string, button?: HTMLElement | null, label = 'テキスト') {
    const origin = document.activeElement instanceof HTMLElement ? document.activeElement : null;
    let state: Feedback | undefined;
    if (button) {
...
export function saveSource(name: string, code: string) {
    downloadBlob(new Blob([code], { type: 'text/plain;charset=utf-8' }), name);
    toast(`${name} のダウンロードを開始しました`, 'success');
}
export function wireTabs(list: HTMLElement, activate: (tab: HTMLElement) => void) {
...
export function required<T extends HTMLElement = HTMLElement>(selector: string, root?: ParentNode): T;
export function required(selector: string, root: ParentNode = document): HTMLElement {
