...
export function buildUsage(part: Part, format: Format, layout: Layout): string {
  ...
  const tabsScroll = part.category === 'tabs' ? '## タブ列のスクロール\n横向きのタブ列は項目そのものが収まらないときだけ横へスクロールできます。選択装飾だけのはみ出しではスクロールバーを出さず、必要なスクロールバーはパーツの素材色と角丸に合わせます。選択面の移動中も縦スクロールバーを出さず、ラベル・フォーカス位置と本文を安定させます。縦向き配置では内容を縦に並べ、スクロール領域を不必要に作りません。\n\n' : '';
