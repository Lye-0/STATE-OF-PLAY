'use client';
import React, {useId,useState,useRef,useEffect,useLayoutEffect,type HTMLAttributes,type ReactNode,type MutableRefObject} from 'react';
...
export function TabsView({items=empty,value,defaultValue,onValueChange,activation='automatic',orientation='horizontal',disabled=false,className='',rootRef,markerArt,panelArt,...attributes}:TabsProps & {rootRef?:MutableRefObject<HTMLDivElement|null>;markerArt?:ReactNode;panelArt?:ReactNode}) {
  ...
  const root=useRef<HTMLDivElement|null>(null),controller=useRef<TabsController|null>(null),id=useId();
  ...
  return <div {...attributes} ref={node=>{root.current=node;if(rootRef)rootRef.current=node;}} className={`sop-choice sop-tabs ${className}`} data-selection-kind="tabs" data-value={selected} data-orientation={orientation} data-disabled={disabled}>
    <div className="sop-choice-list" role="tablist" aria-label={attributes['aria-label']??'内容の切り替え'} aria-orientation={orientation}>
      <span className="sop-choice-marker" aria-hidden="true">{markerArt}</span>
    ...
    <div className="sop-choice-panels">{items.map((item,index)=><section key={item.value} className="sop-choice-panel" data-panel-value={item.value} role="tabpanel" id={`${id}-panel-${index}`} aria-labelledby={`${id}-tab-${index}`} tabIndex={0} hidden={selected!==item.value}>{panelArt}{item.content}</section>)}</div>
