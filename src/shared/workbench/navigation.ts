...
export function createNavigation(root:HTMLElement,provided:NavigationOptions={}):WorkbenchAPI<NavigationOptions,NavigationState>{
 ...
 function marker(){const box=host.querySelector<HTMLElement>('.wb-nav-desktop')!,m=host.querySelector<HTMLElement>('.wb-nav-marker')!;const current=[...box.querySelectorAll<HTMLElement>('[data-nav]')].find(x=>x.dataset.nav===active);if(!current||!current.getClientRects().length){m.hidden=true;return;}const r=current.getBoundingClientRect(),b=box.getBoundingClientRect();m.hidden=false;m.style.width=r.width+'px';m.style.height=r.height+'px';m.style.transform=`translate(${r.left-b.left+box.scrollLeft}px,${r.top-b.top+box.scrollTop}px)`;}
 function paint(){root.dataset.wbLayout=options.layout??'header';root.dataset.wbOpen=String(mobileOpen);host.querySelectorAll<HTMLElement>('[data-nav]').forEach(a=>{if(a.dataset.nav===active)a.setAttribute('aria-current','page');else a.removeAttribute('aria-current');});host.querySelector<HTMLElement>('.wb-nav-location')!.textContent=allItems(items).find(x=>x.id===active)?.label??'Navigation';host.querySelector<HTMLButtonElement>('.wb-nav-mobile-open')!.setAttribute('aria-expanded',String(mobileOpen));marker();}
 ...
 host.addEventListener('focusout',()=>queueMicrotask(()=>{if(!life.dead&&group&&!host.contains(document.activeElement))closeGroup();}),{signal:life.signal});
