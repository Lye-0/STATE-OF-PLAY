...
export function SegmentView({items=empty,value,defaultValue,onValueChange,name,form,required=false,disabled=false,orientation='horizontal',className='',rootRef,markerArt,...attributes}:SegmentProps & {rootRef?:MutableRefObject<HTMLDivElement|null>;markerArt?:ReactNode}) {
  ...
  const id=useId(),root=useRef<HTMLDivElement|null>(null),marker=useRef<ReturnType<typeof createSelectionIndicator>|null>(null);
  ...
  return <div {...attributes} ref={node=>{root.current=node;if(rootRef)rootRef.current=node;}} className={`sop-choice sop-segments ${className}`} data-selection-kind="segments" data-value={selected} data-orientation={orientation} data-disabled={disabled}>
