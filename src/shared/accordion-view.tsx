...
export function AccordionView(props:AccordionProps) {
  const {items=[],expanded,defaultExpanded=[],onExpandedChange,multiple=false,collapsible=true,disabled=false,headingLevel=3,className='',...attributes}=props;
  ...
  return <div {...attributes} ref={root} className={`sop-accordion ${className}`} data-sop-accordion="" data-multiple={multiple}>
    {items.map((item,index)=>{const open=activeValues.includes(item.value);return <section key={item.value} className="sop-accordion-item" data-value={item.value} data-open={open}>
      <Heading className="sop-accordion-heading"><button type="button" id={`${id}-heading-${index}`} className="sop-accordion-trigger" aria-expanded={open} aria-controls={`${id}-panel-${index}`} disabled={disabled||item.disabled}>
