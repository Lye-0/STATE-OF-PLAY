...
import type {SelectController} from './select-controller';
import {createKineticSelect,type KineticMenu} from './kinetic-select';
...
export interface SelectProps extends Omit<HTMLAttributes<HTMLDivElement>,'onChange'|'defaultValue'> {
  ...
  /** Optional initials fallback. Expressive skins default to explicit icons only. */
  autoIcon?:boolean;
  showHeading?:boolean; showHints?:boolean;
...
function OptionCopy({item,autoIcon=true}:{item:SelectItem;autoIcon?:boolean}) {
  return <>{(item.icon || autoIcon) && <span className="sop-select-icon" aria-hidden="true">{item.icon ?? item.label.slice(0,2)}</span>}<span className="sop-select-option-copy"><b>{item.label}</b>{item.description && <small>{item.description}</small>}</span></>;
}
/** React owns the committed value and options. The controller owns popup/focus state only. */
export function KineticSelectView(props:SelectProps & {effect:KineticMenu}) {
  const {effect,items=[],value,defaultValue,onValueChange,onOpenChange,label='選択してください',placeholder='選択してください',name,disabled=false,renderOption,autoIcon=false,showHeading=false,showHints=false,className='',...attributes}=props;
  ...
  useEffect(()=>{
    ...
    const c=createKineticSelect(root.current,effect,{value:initial.current,controlled:true,manageContent:false,
  ...
  return <div {...attributes} ref={root} className={`sop-select sop-kinetic-select ${className}`} data-value={selected} data-placeholder={placeholder}>
    ...
    <button className="sop-select-trigger" type="button" role="combobox" aria-label={label} aria-expanded={false} aria-haspopup="listbox" aria-controls={`${id}-list`} disabled={disabled}>
      <span className="sop-select-value" data-glyph={items.findIndex(i=>i.value===selected)%4}>{chosen?<OptionCopy item={chosen} autoIcon={autoIcon}/>:placeholder}</span><span className="sop-select-chevron" aria-hidden="true"/>
    </button>
    ...
    <div className="sop-select-popup" id={`${id}-list`} role="listbox" aria-label={label} hidden>
      {showHeading && <div className="sop-select-menu-heading" role="presentation"><span>{label}</span><small>{String(items.length).padStart(2,'0')} OPTIONS</small></div>}
      {items.map((item,index)=><React.Fragment key={item.value}>
        ...
        <div className="sop-select-option" id={`${id}-option-${index}`} role="option" data-value={item.value} data-label={item.label} data-glyph={index%4} aria-selected={item.value===selected} aria-disabled={item.disabled||undefined}>
          {renderOption?renderOption(item):<OptionCopy item={item} autoIcon={autoIcon}/>}<span className="sop-select-badge">{item.badge}</span><span className="sop-select-check" aria-hidden="true">✓</span>
        </div>
      </React.Fragment>)}
      {!items.length&&<div className="sop-select-empty">選択肢がありません</div>}
      {showHints && <div className="sop-select-menu-footer" role="presentation"><span>↑ ↓ 選択</span><span>↵ 決定</span><span>esc 閉じる</span></div>}
    </div>
  </div>;
}
