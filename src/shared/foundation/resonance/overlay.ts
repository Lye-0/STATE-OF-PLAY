...
export function resonanceOverlay(c:Core,panel:HTMLElement,anchor:HTMLElement,preference:()=> 'top'|'bottom'=()=> 'bottom',onOutsideScroll?:()=>void) {
 ...
 document.addEventListener('scroll',event=>{if(!opened)return;if(onOutsideScroll){if(!(event.target instanceof Node&&panel.contains(event.target)))onOutsideScroll();}else position();},{capture:true,passive:true,signal:life.signal});
