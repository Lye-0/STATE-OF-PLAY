'use client';
import React, {useEffect,useLayoutEffect,useRef,useState,useId,type HTMLAttributes,type ReactNode} from 'react';
...
export function PopupView({title,description,kicker='DETAILS',children,footer,triggerLabel='詳細を開く',open,defaultOpen=false,onOpenChange,onClose,disabled=false,closeOnBackdrop=true,closeOnEscape=true,confirmLabel='確認する',cancelLabel='閉じる',className='',...attributes}:PopupProps) {
  const [internal,setInternal] = useState(defaultOpen), visible = open ?? internal;
  const root = useRef<HTMLDivElement>(null), control = useRef<PopupController|null>(null), id = useId();
  ...
  return <div {...attributes} ref={root} className={`sop-popup ${className}`}>
